...
const identitySignals = [
  ...
  },
  {
    ...
    story:
      "Discipline is not a motivational quote for me. It is physical. It came from repetition, training, correction, fatigue, and the silent work nobody claps for. Karate made confidence feel earned, not borrowed. That same pattern now moves into learning, coding, design, and building — repeat, refine, sharpen, return.",
  ...
  },
  {
    ...
    story:
      "Voice is the bridge between thinking and impact. Public speaking trained me to hold an idea in front of people and make it clear enough to land. This matters because building is not only about making things; it is also about explaining why they matter, what they change, and why someone should care.",
    ...
    >
      ...
      >
        ...
        <div className="relative flex flex-col justify-center p-6 sm:p-8 md:p-12">
          ...
          >
            <span className="block -translate-y-[1px] text-[30px] font-extralight leading-none">×</span>
          </button>
...
function SplitFlipCard({ signal, index, progress, onOpen }) {
  const x = useTransform(progress, [0, 0.28, 0.36, 0.5, 0.78, 0.86, 0.96, 1], [joinedX[index], joinedX[index], joinedX[index], tableX[index], tableX[index], tableX[index], joinedX[index], joinedX[index]]);
  const y = useTransform(progress, [0, 0.28, 0.36, 0.5, 0.78, 0.86, 0.96, 1], [0, 0, 0, tableY[index] + 72, tableY[index] + 72, tableY[index] + 72, 0, 0]);
  const rotateZ = useTransform(progress, [0, 0.28, 0.36, 0.5, 0.78, 0.86, 0.96, 1], [0, 0, 0, tableRotateZ[index], tableRotateZ[index], tableRotateZ[index], 0, 0]);
  ...
  const joinedRadius = index === 0 ? "32px 0px 0px 32px" : index === 3 ? "0px 32px 32px 0px" : "0px";
  ...
  const border = useTransform(progress, [0, 0.36, 0.5, 0.86, 0.96, 1], ["1px solid rgba(255,255,255,0)", "1px solid rgba(255,255,255,0)", "1px solid rgba(255,255,255,0.14)", "1px solid rgba(255,255,255,0.14)", "1px solid rgba(255,255,255,0)", "1px solid rgba(255,255,255,0)"]);
  const shadow = useTransform(progress, [0, 0.36, 0.5, 0.86, 0.96, 1], ["0px 0px 0px rgba(0,0,0,0)", "0px 0px 0px rgba(0,0,0,0)", "0px 38px 130px rgba(0,0,0,0.62)", "0px 38px 130px rgba(0,0,0,0.62)", "0px 0px 0px rgba(0,0,0,0)", "0px 0px 0px rgba(0,0,0,0)"]);
  ...
  return (
    <motion.button
      ...
      style={{ width: panelWidth, height: posterHeight, x, y, rotateZ, scale, pointerEvents, zIndex: 20 + index, perspective: 1400, transformStyle: "preserve-3d", translateX: "-50%", translateY: "-50%", borderRadius: radius }}
    >
      ...
      <motion.div className="relative h-full w-full" style={{ rotateY, transformStyle: "preserve-3d", borderRadius: radius }}>
        <motion.div
          ...
          style={{ borderRadius: radius, boxShadow: shadow, border, backgroundImage: `url(${posterAsset})`, backgroundSize: `${posterWidth}px ${posterHeight}px`, backgroundPosition: `-${index * panelWidth}px 0px`, backgroundRepeat: "no-repeat" }}
        />
        <motion.div
        ...
        >
          ...
          <div className="relative flex flex-1 flex-col p-4">
            ...
            <h3 className="mt-3 text-[clamp(1.45rem,2vw,2.1rem)] font-black uppercase leading-[0.84] tracking-[-0.085em] text-white">{signal.title}</h3>
      ...
      <FadeIn>
        <div>
          ...
          <div className="noise relative overflow-hidden rounded-[2rem] border border-white/14 bg-[#070707] shadow-[0_44px_170px_rgba(0,0,0,0.62)]">
            <img src={posterAsset} alt="Before the builder, there was the pattern." className="w-full" />
          </div>
        </div>
      </FadeIn>
    );
  ...
  return (
    <div ref={ref} className="relative hidden min-h-[300vh] md:block">
      <div className="sticky top-0 grid h-screen place-items-center overflow-visible">
        <div className="relative h-[min(64vh,620px)] w-[min(92vw,1100px)]" style={{ perspective: 1400, transformStyle: "preserve-3d" }}>
    ...
    <div className="sticky top-20 flex min-h-[82svh] items-start justify-center py-3">
      ...
      >
        ...
        <div className="relative p-4">
          ...
          <h3 className="mt-5 text-[clamp(2.05rem,12vw,3.2rem)] font-black uppercase leading-[0.84] tracking-[-0.09em] text-white">{signal.title}</h3>
  ...
  return (
    <div ref={ref} className="relative px-5 py-16 md:hidden">
      <div className="mx-auto max-w-md">
        <FadeIn>
          <p className="mb-3 text-[10px] font-black uppercase tracking-[0.28em] text-white/45">Identity</p>
          <h2 className="text-[clamp(2.7rem,15vw,4.4rem)] font-black uppercase leading-[0.84] tracking-[-0.09em] text-white">The pattern behind the proof.</h2>
          <p className="mt-5 text-base font-light leading-relaxed text-white/70">Before I became interested in AI, software, and product building, I was shaped by structure, discipline, voice, and execution.</p>
        </FadeIn>
        <div className="mt-10"><CinematicSplitIntroCard mobile /></div>
        <div className="mt-8 pb-[70svh]">
          {identitySignals.map((signal, index) => <MobileStackCard key={signal.title} signal={signal} index={index} progress={scrollYProgress} onOpen={() => onOpen(index)} />)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative hidden px-5 py-10 sm:px-8 sm:py-14 md:block md:px-10">
      <div className="relative mx-auto max-w-[1680px]">
        <FadeIn>
          <p className="mb-3 text-xs font-black uppercase tracking-[0.32em] text-white/45">Identity</p>
          <h2 className="max-w-6xl text-[clamp(2.7rem,9.2vw,6rem)] font-black uppercase leading-[0.84] tracking-[-0.09em] text-white">The pattern behind the proof.</h2>
          <p className="mt-5 max-w-4xl text-xl font-light leading-relaxed text-white/72">Before I became interested in AI, software, and product building, I was shaped by structure, discipline, voice, and execution.</p>
          <p className="mt-4 max-w-4xl text-base leading-relaxed text-white/56">The environments changed — training grounds, uniforms, stages, classrooms, and laptops — but the pattern stayed the same: learn under pressure, communicate with clarity, and turn intent into visible work.</p>
        </FadeIn>
        <div className="mt-6"><CinematicSplitIntroCard onOpen={onOpen} /></div>
      </div>
    </div>
  );
}
